...
import {getTokenStorageType} from "./oauth.js"
import type {TokenStorage} from "./oauth/tokenStorage.js"
import {browserCookie} from "./oauth/tokenStorage/browserCookie.js"
import {localStorage} from "./oauth/tokenStorage/localStorage.js"
import {getResponseCookie, serverCookie, setRequestCookies} from "./oauth/tokenStorage/serverCookie.js"
...
export const svelteKitStrategy: ContextStrategy = new class implements ContextStrategy {
    private fetchFunc: ((arg0: string, arg1: Record<string, unknown> | undefined) => Promise<Response>) | undefined
    private redirectedTo: any = null
    private queryObject: URLSearchParams|null = null

    fetch(uri: string, options?: Record<string, unknown>): Promise<Response> {
        return  this.fetchFunc ? this.fetchFunc(uri, options) : new Promise(() => {})
    }
    ...
    async handleHook({request, resolve}: { request: any; resolve: any }) {
        const env = await import("$app/environment")
        ...
        return Promise.resolve(response).then((response: any) => {
